...
def apply_audio_post_processing(input_path, output_path=None, niche_name=None):
    ...
    try:
        ...
        if Path(output_path).exists() and Path(output_path).stat().st_size > 500000:
            ...
            # verify duration before trusting it.
...
def generate_episode_hashtags(niche, topic):
    ...
    try:
        ...
        raw_tags = ai(tag_prompt, tokens=30, min_chars=6) or ""
...
def main():
    ...
    def _desc_gen(_n, _t, _ti, _ep, _ch, _dur):
        try:
            ...
            _hook = ai(_hook_prompt, tokens=120, min_chars=40, prefer="groq")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                # generic-filler fallback that used to paper over this is
                # gone, so skip the post instead of publishing a placeholder.
...
def main_with_retry():
    ...
    for attempt in range(1, max_retries+1):
        try:
            ...
            # FIX (found diagnosing real upload failures, run 30036342355):
