...
_UNSETTLING_COVERT = [
    "en-GB-ThomasNeural", "en-IE-ConnorNeural", "en-AU-NatashaNeural",
...
SEVEN-STAGE CONTROL STRUCTURE — write continuously, no labels. The stage
            ...
            if new_scenes and len(new_scenes) >= 10:
                ...
                if previous_scenes and new_scenes == previous_scenes:
                    log("  SWAP VISUALS returned an array identical to the "
                        ...
                        "claiming a change.")
                    tg("\u26a0\ufe0f Control Files: SWAP VISUALS produced scenes identical to the "
...
# FIX (direct user report, July 24 2026 — same fix/rationale as Ch2 —
# "blend live action as well as stock inserts for tangibility... use
# both according to the topics... whenever it is required and whenever
# there is demand"): real stock-footage insert mixed in alongside the
# existing polished sketch/motion-graphics scenes for the tangible scene
# types (sketch_document = case files/documents, sketch_findings =
# physical evidence items) — never replacing all of them, since the
# whiteboard reveal is itself real, retention-tested content.
_TANGIBLE_SCENE_TYPES = {"sketch_document", "sketch_findings"}
...
def run_stage3_audio(script_clean, voice_id, niche_name):
    ...
    tg("\U0001f6a8 The Control Files: every human voice failed today. There is no robotic "
       "fallback any more, by design \u2014 today's episode is skipped rather "
       "than narrated by a machine voice. Check provider status urgently.")
...
#     DECLARE_SYNTHETIC_MEDIA_CONTROL_FILES=true   (this channel)
...
DECLARE_SYNTHETIC_MEDIA = declare_synthetic_media("control_files")
    ...
    init = requests.post(
        ...
        json={"snippet": {"title": title[:100], "description": description,
              ...
              "status": {
                  ...
                  # narration over motion graphics and unaltered stock. See
                  # video_pipeline/synthetic_media_policy.py for the policy,
                  # the per-channel assessment, and the trip-wires that mean
                  # this has to go back to True.
...
def main():
    ...
    while _voice_is_robotic(voice_used) and _audio_retry_count < 2:
    ...
    while True:
        ...
        if audio_decision["decision"] == "edit":
            ...
            _edit_retry = 0
            while _voice_is_robotic(voice_used) and _edit_retry < 2:
                _edit_retry += 1
...
def main_with_retry():
    ...
    for attempt in range(1, max_retries+1):
        try:
            ...
            # FIX (found diagnosing real upload failures, run 30040676535):
