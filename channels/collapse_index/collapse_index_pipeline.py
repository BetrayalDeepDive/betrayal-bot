#!/usr/bin/env python3
"""
THE COLLAPSE INDEX — ANIMATED PIPELINE v2.0
...
✅ Animated scenes: resistance_timeline, document, compliance_chart, influence_map, thought_control
...
NUMBER_NOUN_BANKS = {
    ...
    "ai_failures":   ["$2.4M GONE","4,380 DAYS","47 REPORTS","$14M FRAUD","12 YEARS"],
    "algorithm_systems": ["14 VICTIMS","23 YEARS","1 FILE","47 CLUES","3 SUSPECTS"],
    "tech_displacement": ["$840M HIDDEN","14 YEARS","23 EMAILS","$2.4B FRAUD","1 MEMO"],
    "algorithmic_society":  ["2.7M FILES","847 ACCOUNTS","1 IP ADDRESS","23 SERVERS","14TB DATA"],
    "ai_failures":    ["847 MEMBERS","14 YEARS","7 STAGES","23 RULES","1 LEADER"],
    "algorithm_systems": ["40M PEOPLE","7 TECHNIQUES","14 YEARS","3 AGENCIES","1 NARRATIVE"],
    "tech_displacement": ["6 PRINCIPLES","847 TARGETS","23 HOURS","7 TRIGGERS","1 CALL"],
    "algorithmic_society":     ["1B PEOPLE","14 MONTHS","3 NETWORKS","23 COUNTRIES","1 LIE"],
}
...
AFFILIATE_REGISTRY = {
    ...
    "nordvpn":      {"url": "https://nordvpn.com/deepdive",          "label": "NordVPN privacy",          "channels": ["collapse_index","collapse_index"]},
...
YT_CLIENT_ID    = os.environ.get("COLLAPSE_YT_CLIENT_ID",  os.environ.get("YOUTUBE_CLIENT_ID",""))
YT_CLIENT_SEC   = os.environ.get("COLLAPSE_YT_CLIENT_SECRET", os.environ.get("YOUTUBE_CLIENT_SECRET",""))
YT_REFRESH      = os.environ.get("COLLAPSE_YT_REFRESH_TOKEN", os.environ.get("YOUTUBE_REFRESH_TOKEN",""))
TG_TOKEN        = os.environ.get("TELEGRAM_TOKEN", "")
TG_CHAT         = os.environ.get("TELEGRAM_CHAT_ID", "")
...
WORK_DIR      = Path("/home/runner/work/collapse_index")
if not WORK_DIR.exists(): WORK_DIR = Path("/tmp/collapse_index")
...
# ── NICHES ────────────────────────────────────────────────
DAY_NICHE = {0:"ai_failures",1:"tech_displacement",2:"algorithm_systems",3:"algorithmic_society",4:"ai_failures"}
...
NICHES = [
    {
        ...
        "dread_style": "competence dread — the sophistication of the failure and the invisibility of its mechanism",
        "implication": "the systems making decisions about your life right now have the same failure modes",
    },
    {
        ...
        "dread_style": "proximity dread — this is not historical, it is running on your phone right now",
        "implication": "you have never had a private thought that was not monetized by someone you never agreed to",
    },
    {
        ...
        "dread_style": "scale dread — the exact numbers of the people already displaced and the timeline for the rest",
        "implication": "the retraining plans that were promised have a documented completion rate of under 4 percent",
    },
    {
        ...
        "dread_style": "invisibility dread — the decision was made by a system that has no face and answers to no one",
        "implication": "the algorithm is not neutral — it is the preferences of whoever designed it, running at scale",
    },
    {
        ...
        "dread_style": "infrastructure dread — the specific single point and the specific documented vulnerability",
        "implication": "every system you depend on has a documented failure mode and a documented reason it has not been fixed",
    },
    {
        ...
        "dread_style": "infrastructure dread — the systems we depend on were compromised years ago and we learned about it last",
        "implication": "the breach was discovered because someone told us — most breaches are never publicly disclosed",
    },
]
...
# v12: SambaNova — added to Ch5 (was only in Ch1 before)
...
# v12: GEMINI_KEY_2 dual-key for Ch5 (doubles Gemini quota)
...
def get_fresh_topic(niche, attempt, intel, used_topics):
    ...
    if not is_archive:
        ...
        prompt = f"""Generate 6 compelling tech collapse investigation topics for "{niche['series']}".
...
def generate_thumbnail_text(niche, topic, intel):
    ...
    prompt = f"""Generate the most psychologically compelling 3-word thumbnail text for a tech collapse investigation video.
...
Generate exactly 5 YouTube title variants for this tech collapse investigation video.
...
def get_niche_voice_style(state):
    ...
    niche_name = DAY_NICHE.get(day,"ai_failures")
...
def build_dread_prompt_er():
    """The Collapse Index uses investigation-specific dread triggers"""
...
def generate_script_and_scenes(niche, topic, style_name, episode, attempt, intel, prev_title=""):
    """
    v2 script generation for Ch5 (The Collapse Index):
...
{{"type":"failure_timeline","duration":8,"title":"CASE TIMELINE","events":["Event 1: date","Event 2: date","Event 3: date","Event 4: date"],"label":"BREACH TIMELINE"}},
{{"type":"data_breach","duration":7,"title":"THE KEY DOCUMENT","lines":["CASE FILE — RESTRICTED","Reference: [case number]","Finding: [key finding]","Status: [outcome]"],"stamp":"CLASSIFIED"}},
{{"type":"collapse_chart","duration":7,"title":"THE NUMBERS","items":["$X.XM","XX YEARS","XXX VICTIMS","XX REPORTS"],"label":"CASE STATISTICS"}},
{{"type":"network_map","duration":8,"title":"THE NETWORK","nodes":["ORIGIN","ENABLER","SYSTEM","OUTCOME"],"label":"HOW IT CONNECTED"}},
{{"type":"system_alert","duration":10,"title":"EVIDENCE SUMMARY","items":["Finding 1","Finding 2","Finding 3","Finding 4"],"label":"SYSTEM EVIDENCE"}}
]}}
...
Write narration first ({MIN_WORDS}-{MAX_WORDS} words), then 10 dashes, then JSON."""
    ...
    # Parse scene JSON
    scenes, title, thumbnail_text, tags = [], f"The Collapse Index: {topic[:45]}", "CASE DOCUMENTED", \
    ...
    if not scenes:
        scenes = [
            {"type":"failure_timeline","duration":8,"title":"CASE TIMELINE",
             "events":["Event 1","Event 2","Event 3","Event 4"],"label":"BREACH TIMELINE"},
            {"type":"data_breach","duration":7,"title":"KEY DOCUMENT",
             "lines":["CASE FILE — RESTRICTED","Reference: CF-2019-447",
                      "Finding: pattern confirmed","Status: under review"],"stamp":"CLASSIFIED"},
            {"type":"collapse_chart","duration":7,"title":"THE NUMBERS",
             "items":["$2.4M","12 YEARS","847 CASES","47 REPORTS"],"label":"STATISTICS"},
            {"type":"network_map","duration":8,"title":"THE NETWORK",
             "nodes":["ORIGIN","ENABLER","SYSTEM","OUTCOME"],"label":"CONNECTION"},
            {"type":"system_alert","duration":10,"title":"EVIDENCE",
    ...
    if len(clean.split()) >= 400:
        clean = _inject_ctas_er(clean, niche.get("name","ai_failures"))
        wc    = len(clean.split())
...
def render_frame_pil(style_name, scene, frame_idx, total_frames, scene_idx, total_scenes):
    ...
    stype = scene.get("type","system_alert")
    ...
    draw.text((30,H-42), "THE COLLAPSE INDEX — CLASSIFIED", font=font_xs, fill=secondary)
    ...
    # Render scene type
    if   stype=="failure_timeline":      _render_resistance_timeline(draw,scene,progress,style,font_md,font_sm,font_xs)
    elif stype=="document":      _render_document(draw,scene,progress,style,style_name,font_md,font_sm,font_mono)
    elif stype=="collapse_chart":   _render_compliance_chart(draw,scene,progress,style,font_lg,font_md,font_sm)
    elif stype=="network_map":_render_influence_map(draw,scene,progress,style,font_md,font_sm)
...
SCENE_MOTION = {
    "failure_timeline":        "zoompan=z='min(zoom+0.0008,1.3)':d=1:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'",
    "data_breach": "zoompan=z='min(zoom+0.001,1.4)':d=1:x='iw/2-(iw/zoom/2)':y='ih*0.3-(ih/zoom*0.3)'",
    "collapse_chart":     "zoompan=z='if(lte(zoom,1.0),1.3,max(1.001,zoom-0.001))':d=1:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'",
    "network_map":  "zoompan=z='1.2+0.05*sin(2*PI*on/100)':d=1:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'",
    "system_alert":  "zoompan=z='min(zoom+0.0006,1.25)':d=1:x='(iw-iw/zoom)*on/n':y='ih/2-(ih/zoom/2)'",
}
...
def apply_ken_burns(input_path, output_path, scene_type, fps=24, duration=None):
    ...
    """
    motion = SCENE_MOTION.get(scene_type, SCENE_MOTION["failure_timeline"])
    try:
        cmd = [
...
def generate_thumbnail_with_ai_bg(title, thumb_text, niche_name, topic,
                                   ab_style="A", episode=1, channel_name="The Collapse Index"):
...
def run_stage3_audio(script_clean, voice_id, niche_name):
        ...
        try:
            ...
            if r.status_code == 200 and len(r.content) > 50000:
                ...
                if check_audio_quality(mp3, dur_expected):
                    ...
                    tg("⚠️ The Collapse Index: all edge-tts voices failed today — used Fish Audio backup instead (still natural-sounding)")
    ...
    try:
        ...
        if parts:
            ...
            if Path(mp3).exists() and Path(mp3).stat().st_size > 50000:
                ...
                tg("⚠️ The Collapse Index: edge-tts AND Fish Audio both failed today — used gTTS backup "
    ...
    try:
        ...
        if Path(wav).exists() and Path(wav).stat().st_size > 50000:
            ...
            tg("🚨 The Collapse Index: ALL providers failed today (edge-tts, Fish Audio, gTTS) — used OFFLINE "
    ...
    tg("The Collapse Index Stage 3 FAILED — all voices AND all backup providers failed")
    sys.exit(1)
...
def fetch_case_relevant_image_ch2(topic, niche_name, out_path):
    ...
    niche_mod   = {
        "ai_failures":       "dark corporate financial documents",
        "algorithm_systems": "dark crime evidence investigation",
        "tech_displacement":     "dark corporate shadow documents",
        "algorithmic_society":      "dark technology screen code shadow",
    }
            ...
            if r.status_code == 200 and r.json().get("hits"):
                ...
                if url:
                    ...
                    if ir.status_code == 200 and len(ir.content) > 20000:
                        ...
                        log(f"  Case image Ch5 (Pixabay): {search_kw}")
                        return True
        except: pass
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if photos:
                    ...
                    if ir.status_code == 200 and len(ir.content) > 20000:
                        with open(out_path, "wb") as f: f.write(ir.content)
                        log(f"  Case image Ch5 (Pexels): {search_kw}")
                        return True
        except: pass
    return False
...
def generate_thumbnail(title, thumb_text, niche_name, topic, ab_style="A",
                        episode=1, channel_name="The Collapse Index"):
...
def post_creator_comment(token, video_id, niche_name, title, episode):
    ...
    niche_hooks = {
        "ai_failures":       "What financial warning sign do you think most people miss?",
        "algorithm_systems": "Which piece of evidence in this case do you find most disturbing?",
        "tech_displacement":     "Have you ever seen this happen at a company you know?",
        "algorithmic_society":      "Did you know your digital footprint tells this much about you?",
    }
    ...
    comment = (
        ...
        f"🔔 New tech collapse investigation every weekday\n"
...
def generate_dedicated_short_title_ch2(main_title, short_type, niche_name):
    """Dedicated Short title for Ch5 — tech collapse investigation angle."""
    prompts = {
        "teaser": f"Write a YouTube Shorts title that creates maximum curiosity for a tech collapse investigation. "
    ...
    try:
        ...
        if result:
            ...
            if 15 < len(title) < 65:
                log(f"  Short title Ch5: {title}")
                return title
    except Exception as e:
        log(f"  Short title Ch5 (non-fatal): {e}")
...
def post_short_creator_comment_ch2(token, video_id, niche_name, main_title):
    """Pinned creator comment on each Ch5 Short. Drives early engagement signals."""
    short_hooks = {
        "ai_failures":       "What financial warning sign do you wish more people understood?",
        "algorithm_systems": "What detail in this case makes it impossible to look away?",
        "tech_displacement":     "Have you ever seen corporate documents like these in real life?",
        "algorithmic_society":      "Did you know how much of your digital trail can be reconstructed?",
    }
    hook = short_hooks.get(niche_name, "What was the most disturbing piece of evidence?")
    comment = (
        f"🔬 {hook}\n\n"
        f"Full tech collapse investigation ↑ above.\n"
        f"🔔 New case every weekday → subscribe\n"
        f"🌑 Dark horror: youtube.com/@BetrayalDeepDive\n"
        f"🧠 Mass manipulation: youtube.com/@TheCollapseIndex\n\n"
    ...
    try:
        r = requests.post(
            ...
            json={"snippet": {"videoId": video_id,
                              "topLevelComment": {"snippet": {"textOriginal": comment}}}},
            timeout=30)
        if r.status_code == 200: log("  Short creator comment Ch5 OK")
...
def build_ch2_cross_promo(is_short=False):
    """Three-channel cross-promotion for Ch5 descriptions."""
    if is_short:
        return (
            "\n\n🌑 Dark horror investigations: youtube.com/@BetrayalDeepDive"
            "\n🧠 Mass manipulation exposed: youtube.com/@TheCollapseIndex"
        )
    return (
        "\n\n🌑 Dark psychological horror: youtube.com/@BetrayalDeepDive"
        "\n🧠 Mass manipulation & algorithm: youtube.com/@TheCollapseIndex"
        "\n\n📺 New investigation every weekday on all three channels."
    )


def track_episode_ch2(state, niche_name, score, voice, episode):
    """Performance tracker for Ch5 — same as Ch1's track_episode."""
    perf = state.get("performance", {})
...
SHORTS_TEMPLATES = {
    "ai_failures":       ["The one financial warning sign that nobody acted on",
                               "The document trail that exposed the entire fraud"],
    "algorithm_systems": ["The single piece of evidence that broke the whole case",
                               "The detail in the scene that proved it was not an accident"],
    "tech_displacement":     ["The internal memo that exposed the cover-up",
                               "The document they tried to destroy and failed"],
    "algorithmic_society":      ["The digital trace that was impossible to erase",
                               "The metadata that revealed the entire resistance_timeline"],
}
...
def create_standalone_short_video(script, audio_path, niche_name, short_num):
    ...
    bg_colors = {
        "ai_failures":       (8, 12, 20),
        "algorithm_systems": (12, 5, 5),
        "tech_displacement":     (5, 8, 12),
        "algorithmic_society":      (5, 15, 10),
    }
    ...
    for section_idx, section_text in enumerate(sections):
        ...
        for fi in range(section_frames):
            ...
            # Channel badge
            draw.text((40, 30), "● THE COLLAPSE INDEX", font=gf(26), fill=(160, 0, 0))
    ...
    for short_num in range(2):
        try:
            ...
            short_title = (
                ...
                if short_num == 0
                else f"THE COLLAPSE INDEX: {topic[:35]} #Shorts"
            )
            short_desc = (
                f"{script[:200]}\n\n"
...
def run_stage1(state):
    """
    13-attempt script engine for Ch5 The Collapse Index.
    Returns all script data needed by generate phase.
    """
    log("\n"+"="*65)
    log("  STAGE 1: The Collapse Index 13-Attempt Script Engine")
...
def run_stage2_approval_ch2(title_str, niche, score, script_clean):
    ...
    msg = (
        f"<b>COLLAPSE INDEX APPROVAL NEEDED</b>\n\n"
        f"Title: {title_str}\n"
...
def _inject_ctas_er(script_clean, niche_name):
    ...
    cta_bank = {
        "ai_failures": [
            ...
            "More documented cases like this are coming. Subscribe to The Collapse Index.",
        ],
        "algorithm_systems": [
            "If this pattern concerns you, subscribe — more documented systems every week.",
            "This channel tracks documented algorithm systems. Subscribe to follow the record.",
            "More documented findings like this are coming. Subscribe to The Collapse Index.",
        ],
        "tech_displacement": [
    ...
    ctas = cta_bank.get(niche_name, cta_bank["ai_failures"])
...
# ═══════════════════════════════════════════════════════════
# PORTED FROM Ch1 — Advanced features for Ch5
# ═══════════════════════════════════════════════════════════
...
def main():
    """
    Two-phase controller for Ch5 (The Collapse Index).
    ...
    log(f"\nCOLLAPSE INDEX v14.0 — Phase: {phase.upper()}")
    ...
    if phase == "upload":
        ...
        if not pending or is_already_uploaded(pending):
            tg("⚠️ Ch5 Upload: no pending video. Generation may have failed.")
            sys.exit(0)
        is_fresh, hours_old = check_pending_age(pending, max_hours=30)
        if not is_fresh:
            tg(f"⚠️ Ch5 Upload: pending is {hours_old}h old — uploading anyway.")
        ...
        if not video_path or not Path(video_path).exists():
            tg(f"❌ Ch5 Upload: video file missing. Run Generate first.")
            sys.exit(1)
        ...
        if script_clean and duration > 0:
            try:
                ...
                upload_srt_captions(token_yt, vid_id, script_clean, duration, "collapse_index")
        ...
        send_hype_push(yt_url, title, "The Collapse Index", day=0)

        tg(f"✅ <b>The Collapse Index — LIVE</b>\n\n"
           f"<b>{title}</b>\n🔗 {yt_url}\n\n"
    ...
    cross_promo     = get_cross_promo("collapse_index", is_short=False)
    affiliate_block = build_affiliate_block("collapse_index", niche["name"])
    ...
    chapters_block = _gen_chapters(script_clean, duration, "collapse_index")
    description = (f"{seo_first}\n\nEpisode {episode} of {niche['series']}.\n\n"
                   f"Every case. Every document. Every piece of evidence — animated.\n\n"
                   f"{chapters_block}\n\n"
                   f"Subscribe to The Collapse Index."
                   f"{cross_promo}"
                   f"{affiliate_block}\n\n"
    ...
    thumb_path = generate_thumbnail_with_ai_bg(
        ...
        episode=episode, channel_name="The Collapse Index")

    # Validate video file before saving to pending
    if not Path(video_path).exists():
        tg(f"❌ Ch5 Generate FAILED: video file not created")
        sys.exit(1)
