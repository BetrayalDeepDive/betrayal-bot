...
def _record_title_history(niche_name, episode, title, score):
    ...
    try:
        ...
        record_title_used(str(SCRIPT_DIR), "TheCollapseIndex", niche_name, episode, title, score)
...
# used; it kept resurfacing because it was hardcoded first in fallback
# lists and present in most niche pools.
EXTENDED_VOICES = [
    "en-GB-ThomasNeural", "en-GB-AlfieNeural",
...
_URGENT_DRAMATIC = [
    ...
    "en-AU-KenNeural", "en-CA-ClaraNeural", "en-IE-EmilyNeural",
    "en-GB-AlfieNeural", "en-NZ-MollyNeural",
]
_WARM_EXPLAINER = [
    "en-AU-NatashaNeural", "en-GB-SoniaNeural", "en-AU-WilliamNeural",
...
def generate_script_content(niche, topic, episode, attempt,
    ...
    # meaning ANY attempt landing even slightly short of that lost
    # stage_texts entirely, and every downstream consumer (PDF export,
    # Telegram review) fell back to flat, unlabeled prose with no error
    # or log line. split_into_stage_texts is a pure, proportional split
    # that works correctly regardless of total length, so there was
    # never a real reason to gate it on MIN_WORDS -- only the heavier
    # stage-scoring/rewrite loop below genuinely needs that gate.
    # Computed here, unconditionally, so review material always shows
    # real structure.
...
def _inject_ctas_ch5(script_clean, niche_name):
    ...
    cta_pool = {
        "ai_startup_collapse": {
            ...
            "60pct": ["Subscribe. What's documented next reveals the final numbers.",
        ...
        "tech_company_collapse": {
            ...
            "60pct": ["Subscribe. What's documented next reveals the numbers.",
        ...
        "crypto_collapse": {
            ...
            "60pct": ["Subscribe. What comes next reveals the full collapse.",
        ...
        "cybersecurity_disasters": {
            ...
            "60pct": ["Subscribe. What comes next reveals the full breach timeline.",
        ...
        "product_flops": {
            ...
            "60pct": ["Subscribe. What comes next reveals the full failure.",
        ...
        "dotcom_era_collapse": {
            ...
            "60pct": ["Subscribe. What comes next reveals the full history.",
        ...
        "personal_finance_mistakes": {
            ...
            "60pct": ["Subscribe. What happens next is the full real breakdown.",
                      "Subscribe to The Collapse Index. What comes next is the complete real numbers."],
...
def run_audio_stage(script, niche_name, edge_voice):
    ...
    if not el_ok:
        _fallback_candidates = [v for v in
            ["en-GB-ThomasNeural", "en-GB-OliverNeural"] + EXTENDED_VOICES if v != edge_voice]
        try:
    ...
    if not Path(audio_path).exists() or Path(audio_path).stat().st_size < 10000:
        ...
        #
        # A gTTS route and an offline espeak-ng route used to sit here. The
        # espeak one said it published "so the video still published", and on
        # this channel that was literally true -- the voice-tier arithmetic
        # that blocks a robotic narration on Ch1 was never propagated here, so
        # a machine voice really could reach this audience.
        #
        # Per instruction, "only humanic voices": both removed. The human
        # routes above remain, and if every one of them fails the episode is
        # skipped rather than narrated by a synthesiser.
        if not fallback_ok:
            tg("\U0001f6a8 The Collapse Index: every human voice failed today. "
               "There is no robotic fallback any more, by design \u2014 today's "
               "episode is skipped rather than narrated by a machine voice.")
...
def get_stage_matched_video(niche, script, audio_duration, chart_data=None, topic="", title=""):
    ...
    # FIX (direct user report, July 24 2026 — same bug/rationale as Ch1):
    # topic_anchors used to be picked by raw word frequency alone, with no
    # check on whether a word is something a generic stock-video library
    # can actually match. Abstract/administrative words return zero real
    # hits once paired with a mood phrase, silently degrading the search
    # down to the fully generic catch-all fallback — which is what actually
    # surfaced unrelated aesthetic clips. Concrete, visually groundable
    # nouns (office, spreadsheet, laptop, warehouse, factory, storefront...
    # plus the general documentary set) now get first priority.
    CONCRETE_VISUAL_NOUNS = {
        ...
        "spreadsheet","spreadsheets","invoice","invoices","contract","contracts","storefront",
        "boardroom","chart","charts","graph","graphs","calculator","cubicle","skyline",
        "stockmarket","exchange","bank","vault","atm","billboard","suitcase",
    }
    ...
    for i in range(n_buckets):
        ...
        # from. The niche's own BG_KEYWORDS entry is now the true last
        # resort before a neutral black clip.
...
#     DECLARE_SYNTHETIC_MEDIA_COLLAPSE_INDEX=true   (this channel)
...
DECLARE_SYNTHETIC_MEDIA = declare_synthetic_media("collapse_index")
    ...
    init = requests.post(
        ...
        json={"snippet": {"title": title[:100], "description": desc,
              ...
              "status": {"privacyStatus": privacy,
                         ...
                         # Was hardcoded True with a comment calling it
                         # "mandatory AI disclosure since Mar 2024" — it is
                         # not mandatory for narration over charts, maps and
                         # unaltered stock. See
                         # video_pipeline/synthetic_media_policy.py for the
                         # policy, the per-channel assessment, and the
                         # trip-wires that mean this has to go back to True.
...
def main():
    ...
    if phase == "upload":
        ...
        # (no Telegram size limit) so the whole thing can be watched/
        # downloaded and judged before it ever goes public.
        ...
        _final_gate = review_final_video_before_publish(
            "Ch5 The Collapse Index", yt_url, thumb_path,
        ...
        if _final_gate["decision"] != "approve":
            ...
            tg(f"🔄 Ch5: final video rejected — unlisted upload removed, "
    ...
    try:
        ...
        while _voice_is_robotic(edge_voice) and \
        ...
        if _voice_is_robotic(edge_voice):
